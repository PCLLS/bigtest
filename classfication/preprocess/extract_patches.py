...
class ExtractPatch:
    ...
    def extract_from_single_slide(self, slidename):
        ...
        for col in range(int((W - self.grid_size / 2) // self.step_size)):
            ...
            for row in range(int((H - self.grid_size / 2) // self.step_size)):
                otsu, white_flag = wsi.read_otsu(slide, x_center, y_center, self.otsu_level, self.win_size,
                                                 self.win_size, self.white_thred)
    ...
    def extract_test_from_single_slide(self, slidename, ):
        '''
        Sample Patch with limited stride for tissue
        :param slide:
        read_slide: return im (height, width X C)
        ...
        W, H = slide.dimensions
        x_center = self.grid_size // 2
        while (w + self.grid_size / 2 < W):
            y_center = self.grid_size // 2
            while (w + self.grid_size / 2 < h):
                otsu, white_flag = wsi.read_otsu(slide, x_center, y_center, self.otsu_level, self.win_size,
                                                 self.win_size, self.white_thred)
                label = Label(mask, x_center, y_center, self.level, win=self.win_size)
                if white_flag or label:
                    ...
                    count += 1
                    stats[label] += 1
                    y_center += self.grid_size
                else:
                    y_center += self.step_size
            x_center += self.grid_size
        logging.info(f'samples {str(stats)} from {slidename} done!!')
        if mask != 0 and stats[1] == 0:
            logging.warning(f'{slidename} cannot find tumor')
        table.to_csv(save, header=True)
...
def get_arg():
    ...
    parser.add_argument("-wt", "--white_thred", type=float, default=0.1, help="train mode or test mode")
...
if __name__ == '__main__':
    ...
    if args.test:
        tif_folder = TESTSET
        extractor = ExtractPatch(tif_folder, mask_folder, level, save_path, StepSize=stepsize, grid_size=grid_size,
                                 win_size=win_size, otsu_level=otsu_level, white_thred=white_thred)
        with ProcessPoolExecutor(max_workers=num_works) as pool:
            futures = [pool.submit(extractor.extract_test_from_single_slide, slide) for slide in extractor.tifs.keys()]
    else:
        tif_folder = TRAINSET
        extractor = ExtractPatch(tif_folder, mask_folder, level, save_path, StepSize=stepsize, grid_size=grid_size,
                                 win_size=win_size, otsu_level=otsu_level, white_thred=white_thred)
