...
class wsi(object):
    ...
    def otsu_rgb(slide,level,save=None):
        ...
        return tissue_mask
    @staticmethod
    def read_otsu(slide,x,y,level,width,height,white=True):
        assert isinstance(slide, openslide.OpenSlide)
        mag = pow(2, level)
        x = int(x - width * mag / 2)
        y = int(y - width * mag / 2)
        ...
        thumbnail = slide.read_region((x, y), level, (width, height)).convert('RGB')
        img_RGB = np.asarray(thumbnail)
        img_HSV = rgb2hsv(img_RGB)
        background_R = img_RGB[:, :, 0] > 203
        background_G = img_RGB[:, :, 1] > 191
        background_B = img_RGB[:, :, 2] > 201
        tissue_RGB = np.logical_not(background_R & background_G & background_B)
        tissue_S = img_HSV[:, :, 1] > 0.1113
        '''如果仅使用threadshold，中间会有部份白色脂肪区域被隔离'''
        rgb_min = 50
        min_R = img_RGB[:, :, 0] > rgb_min
        min_G = img_RGB[:, :, 1] > rgb_min
        min_B = img_RGB[:, :, 2] > rgb_min
        tissue_mask = tissue_S & tissue_RGB & min_R & min_G & min_B
        assert tissue_mask.shape == (width, height)
    ...
    @staticmethod
    def read_mask(slide, x, y, level, width, height, as_numpy=False):
        '''
