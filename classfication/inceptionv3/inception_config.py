...
dataset = ListDataset(TRAINSET,MASK_FOLDER,level,patch_size * pow(2,level),crop_size * pow(2,level),table) # 训练集所有数据导入
# 随机分割验证集和训练集
...
train_sampler=RandomSampler(data_source=dataset,slides=train_slides,num_samples=3200)
